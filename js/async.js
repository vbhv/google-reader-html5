...
async = function async (fn) {
	return function () {
		...
		_dbg("called, and got " + objToStr.call(gen));
		if (objToStr.call(gen) === "[object Generator]") {
			var callback = function (response) {
				try {
					_dbg("sending response: " + response + " to gen: " + gen + " (len " + gen.length + ")");
					var descriptor = gen.send(response);
					if (descriptor) {
						_dbg("forwarding to descriptor[0]: " + descriptor[0]);
						descriptor[0](descriptor[1], callback);
						_dbg("done")
					} else {
						_dbg("response " + response + " is not going anywhere!");
					}
...
req = async.request = (function () {
	if (XHR) {
		...
		return function ({0: url, 1: method, 2: data, length: argsLen}, callback) {
			var req = new XHR;
			req.open((method || "GET"), url + (("?" + data) || ""), True);
